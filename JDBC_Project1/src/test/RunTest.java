...
public class RunTest {
	/*
	 * JDBC를 사용하기 위해서는 자바 프로젝트에 jdbc드라이버를 추가해 줘야 한다.
	 * 
	 * -JDBC용 객체 _Connection : DB의 연결 정보를 담고있는 객체 
	 * -[Prepared]Statement : 연결된 DB에 sql문을 전달해서 실행하고, 결과를 받아내는 객체
	 * -Result  : SELECT문 실행 후 조회된 결과물을 담는 객체
	 * 
	 * *JDBC 과정(순서중요)
	 * 1) JDBC Driver등록 : 해당 DBMS(오라클)가 제공하는 클래스 등록
	 * 2) Connection생성 : 연결하고자하는 db정보를 입력해서 해당 db와 연결할 수 있는 객체 생성
	 * 3) Statement생성 : Connection객체를 이용해서 생성(sql문 실행칭 결과를 받는 객체)
	 ...
	 * 
	 * 
	 * 6_1) ResultSet객체에 담겨있는 데이터들을 하나씩 추출해서 차근차근 옮겨담기[+ArrayList에 담기]
	...
	public static void main(String[] args) {
		...
		Statement stmt = null; //sql문을 전달해서 실행 후 결과를 받는 객체 (import 할 때 주의!반드시 sql로 접속 해야한다) 
		...
		try {
			//1) JDBC Driver등록 
			Class.forName("oracle.jdbc.driver.OracleDriver"); // ojdbc.jar 파일을 추가하지 않았을까 확인 | 오타 =>  에러
		...
		*/
		...
		// 2. 내 PC DB상에 JDBC계정에 TEST 테이블에 모든 데이터 조회해보기
		...
		//rset.next() => rset의 다음행이 들어있는지 없는지 확인 후  결과반환 + 한행을 내려줍니다
		
		while (rset.next()) {
			//현재 참조하는 rset으로부터 어떤 컬럼에 해당하는 값을 어떤타입으로 추출할건지 제시해 줘야한다.
