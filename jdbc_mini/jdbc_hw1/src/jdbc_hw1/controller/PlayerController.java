...
public class PlayerController {
	...
	public void selectByPlayerName1(String playerName) {
		Player p = new PlayerService().selectByPlayerName(playerName);
		if (p == null) {
			new PlayerMenu().displayNoData(playerName + "에 해당하는 조회결과가 없습니다. 다시 입력해주세요");
		} else {
			if(p.getAge() >= 18) {
			new PlayerMenu().displaySuccess(playerName + "은(는) 성인선수입니다\n");
			} else {
				new PlayerMenu().displaySuccess(playerName + "은(는) 청소년입니다\n");
			}
	}
